...
class CreateChatUser(forms.ModelForm):
    ...
    ChatUser_password = forms.CharField(widget=forms.PasswordInput, help_text="Please enter your password",
                                        label="Password")

    class Meta:
        model = ChatUser
        fields = {"ChatUser_nick", "ChatUser_fname", "ChatUser_lname", "ChatUser_email", "ChatUser_dob",
                  "ChatUser_password"}

    def clean_rec_data(self):
        full_data = self.cleaned_data[
            'ChatUser_nick', 'ChatUser_fname', 'ChatUser_lname', 'ChatUser_email', 'ChatUser_dob',
            'ChatUser_password']
        return full_data
